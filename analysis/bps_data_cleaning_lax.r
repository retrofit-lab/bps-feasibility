## -----------------------------------------------------------------------------------------------
## Title: bps_data_cleaning_lax.R
## Purpose: Cleans and filters benchmarking data from Los Angeles 
...
all_bench <- read_csv("Existing_Buildings_Energy___Water_Efficiency__EBEWE__Program.csv",
...
minus_blank_bench <- minus_blank_bench %>% 
  rename(facility_type = `PROPERTY TYPE`,
		 floor_area = `GROSS BUILDING FLOOR AREA (ftÂ²)`,
		 site_eui = `SITE ENERGY USE INTENSITY (EUI) (kBtu/ftÂ²)`,
		 property_name = `BUILDING ID`, # Note that this is property ID, not property name
...
write_csv(export_bench, "../cleaned_data/cleaned_los_angeles.csv")
