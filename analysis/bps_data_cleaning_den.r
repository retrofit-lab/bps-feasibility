## -----------------------------------------------------------------------------------------------
## Title: bps_data_cleaning_den.R
## Purpose: Cleans and filters benchmarking data from Denver
...
all_bench <- read_csv("2019 Final Master Dataset.csv")
...
minus_blank_bench <- minus_blank_bench %>% 
  rename(facility_type = `Primary Property Type - EPA Calculated`,
		 floor_area = `Master Sq Ft`,
		 site_eui = `Site EUI (kBtu/sq ft)`,
...
filtered_subset_bench <- filtered_subset_bench %>% 
  rename(
    electricity = `Electricity Use - Grid Purchase (kWh)`,
  ...
  mutate_at(c("electricity", "natural_gas", "district_steam", "district_chw"), ~replace_na(.,0))	
	
# Calculate site EUI from total energy use and find difference with reported site EUI 
filtered_subset_bench <- filtered_subset_bench %>% 
  mutate(
	total_site_energy = (electricity * 3.412) + natural_gas + district_steam + district_chw,
...
write_csv(export_bench, "../cleaned_data/cleaned_denver.csv")
