## -----------------------------------------------------------------------------------------------
## Title: bps_data_cleaning_sea.R
## Purpose: Cleans and filters benchmarking data from Seattle
...
all_bench <- read_csv("2019_Building_Energy_Benchmarking.csv",
...
minus_blank_bench <- minus_blank_bench %>% 
  rename(facility_type = EPAPropertyType,
		 floor_area = `PropertyGFABuilding(s)`,
		 site_eui = `SiteEUI(kBtu/sf)`,
...
minus_blank_bench <- minus_blank_bench %>% 
  mutate(facility_type = recode_factor(facility_type, "Multifamily LR (1-4)" = "Multifamily Housing",
	"Multifamily MR (5-9)" = "Multifamily Housing", "Multifamily HR (10+)" = "Multifamily Housing"))
...
# Eliminate commas additional variables of interst 
filtered_subset_bench$`SteamUse(kBtu)` <- gsub(',', '', filtered_subset_bench$`SteamUse(kBtu)`)
filtered_subset_bench$`NaturalGas(kBtu)` <- gsub(',', '', filtered_subset_bench$`NaturalGas(kBtu)`)
filtered_subset_bench$`Electricity(kBtu)` <- gsub(',', '', filtered_subset_bench$`Electricity(kBtu)`)

# Rename additional variables of interst 
filtered_subset_bench <- filtered_subset_bench %>% 
  rename(
	district_steam = `SteamUse(kBtu)`,	
  ...
  mutate_at(c("district_steam", "natural_gas", "electricity"), ~replace_na(.,0))	

# Calculate site EUI from total energy use and find difference with reported site EUI 
filtered_subset_bench <- filtered_subset_bench %>% 
  mutate(
	total_site_energy = electricity + natural_gas + district_steam,
...
# Computed percentiles for reported vs. calculated site EUI to find systematic bias
pct_eui_reported <- filtered_subset_bench %>% 
...
pct_eui_calculated <- filtered_subset_bench %>% 
  group_by(facility_type) %>%
  summarize(pct_10 = quantile(site_eui_calc, prob = 0.10),
			pct_25 = quantile(site_eui_calc, prob = 0.25),
			pct_50 = quantile(site_eui_calc, prob = 0.50),
			pct_75 = quantile(site_eui_calc, prob = 0.75),
			pct_90 = quantile(site_eui_calc, prob = 0.90))
...
write_csv(export_bench, "../cleaned_data/cleaned_seattle.csv")
