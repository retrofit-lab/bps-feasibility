## -----------------------------------------------------------------------------------------------
## Title: bps_data_cleaning_bos.R
## Purpose: Cleans and filters benchmarking data from Boston 
...
all_bench <- read_csv("berdo-data-for-disclosure-calendar-year-2018.csv")
...
filtered_subset_bench <- filtered_subset_bench %>% 
  rename(total_site_energy = `Total Site Energy (kBTU)`) 

# Calculate site EUI from total energy use and find difference with reported site EUI 
filtered_subset_bench <- filtered_subset_bench %>% 
  mutate(
    total_site_energy = as.numeric(total_site_energy),
...
write_csv(export_bench, "../cleaned_data/cleaned_boston.csv")
