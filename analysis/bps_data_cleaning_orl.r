## -----------------------------------------------------------------------------------------------
## Title: bps_data_cleaning_orl.R
## Purpose: Cleans and filters benchmarking data from Orlando
...
all_bench <- read_csv("BEWES_Building_Data.csv",
...
minus_blank_bench <- minus_blank_bench %>% 
  rename(facility_type = `Primary Use Type`,
		 floor_area = `Building Size`,
		 site_eui = `Site Energy Use Intensity (Site EUI)`,
...
write_csv(export_bench, "../cleaned_data/cleaned_orlando.csv")
