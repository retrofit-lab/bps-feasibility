...
cleaned_all_cities <- list.files(pattern='csv') %>% 
  ...
  bind_rows()
```
The combined dataset is then exported to the the directory `../aggregated_data/` as the file [cleaned_all_cities.csv](data/aggregated_data/cleaned_all_cities.csv).  Data for St. Louis have been omitted from this file, since the benchmarking data for St. Louis are not publicly available.  
